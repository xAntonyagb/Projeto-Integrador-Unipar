...
public class BlocoRepositoryImpl implements ICustomRepository<BlocoEntity> {

    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public Page<BlocoEntity> findAllWithFilters(Pageable pageable, Map<String, String> filtros) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<BlocoEntity> query = cb.createQuery(BlocoEntity.class);
        Root<BlocoEntity> bloco = query.from(BlocoEntity.class);
        List<Predicate> listWhere = new ArrayList<>();

        CriteriaQuery<Long> queryCount = cb.createQuery(Long.class);
        Root<BlocoEntity> blocoCount = queryCount.from(BlocoEntity.class);
        List<Predicate> listWhereCount = new ArrayList<>();

        filtros.forEach((key, value) -> {
            if (key.equals("descricao") ) {
                final String descricao = value.toUpperCase();
                listWhere.add(cb.like(cb.upper(bloco.get("descricao")), "%" + descricao + "%"));
                listWhereCount.add(cb.like(cb.upper(blocoCount.get("descricao")), "%" + descricao + "%"));
            }
        });

        query.where(listWhere.toArray(new Predicate[0]));

        TypedQuery<BlocoEntity> consultaPaginada = entityManager.createQuery(query); //rodou a query
        consultaPaginada.setFirstResult((int) pageable.getOffset());
        consultaPaginada.setMaxResults(pageable.getPageSize());

        List<BlocoEntity> resultado = consultaPaginada.getResultList();

        //Pra dar count na quantidade de registros que tem com esse filtro
        queryCount.select(cb.count(blocoCount)).where(listWhereCount.toArray(new Predicate[0]));
