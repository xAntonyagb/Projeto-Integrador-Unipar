...
export class ApiGenericToasts {
  ...
  showErro(response: any) {
    ...
    switch (response.status) {
      case 500:
        this.toastr.error(
          `Ops! houve um erro interno no servidor. Por favor tente mais tarde.`,
          '',
      ...
      case 404:
        if (listErros.length > 0) {
          for (let i = 0; i < listErros.length; i++) {
            this.toastr.info(String(listErros[i]), '', {
              timeOut: 2000,
              progressBar: true,
            });
          }
        } else {
          this.toastr.warning(`Nenhum registro foi encontrado.`, '', {
            timeOut: 2000,
            progressBar: true,
          });
        }
      ...
      case 400:
        if (listErros.length > 0) {
          for (let i = 0; i < listErros.length; i++) {
            this.toastr.error(String(listErros[i]), '', {
              timeOut: 4000,
              progressBar: true,
            });
          }
          ...
          this.toastr.error(
            `Erro ao requisitar informações para o servidor. Confira o console para mais detalhes.`,
            '',
            { timeOut: 4000, progressBar: true },
          );
        }
      ...
      default:
        try {
          for (let i = 0; i < listErros.length; i++) {
            this.toastr.warning(String(listErros[i]), '', {
              timeOut: 4000,
              progressBar: true,
            });
          }
          ...
          this.toastr.error(
            `Ops! Um erro inesperado aconteceu. Confira o console para mais detalhes.`,
            '',
            { timeOut: 4000, progressBar: true },
          );
        }
